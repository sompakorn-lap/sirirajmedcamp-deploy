...
const getQuestions = asyncHandler(async (req, res) => {
  ...
  const already = await MedicalExamAnswer.findOne({ userId }).lean().exec()
  if(already){
    if(already.editable){
  ...
  let queryQuestions = await MedicalExamQuestion.find({ questionSet }).lean()

  const questions = await MedicalExamAnswer.create({
    userId,
    editable: true,
    recentlyIndex: 0,
...
const saveAnswers = asyncHandler(async (req, res) => {
  ...
  const answer = await MedicalExamAnswer.findOneAndUpdate({ userId, editable: true }, { answers, recentlyIndex }).lean().exec()
  if(!answer){
...
const submitAnswers = asyncHandler(async (req, res) => {
  ...
  const answer = await MedicalExamAnswer.findOneAndUpdate({ userId, editable: true }, { editable: false }).lean().exec()
  if(!answer){
    return res.status(404).send('Answer not found')
  }
